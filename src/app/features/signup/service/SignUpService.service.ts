...
export class SignUpService {
  ...
  checkServiceArea(
    ...
    return new Observable((observer) => {
      ...
      this.httpClient.post<CheckServiceAreaResponse>(url, data).subscribe(
        (response) => {
  ...
  fetchSubscriptionPlans(): Observable<Array<NewUserSubscriptionPlan>> {
    return new Observable((observer) => {
      ...
      this.httpClient.get<Array<NewUserSubscriptionPlan>>(url).subscribe(
        (subscriptionPlans) => {
  ...
  geocodeLocation(lat: number, lng: number): Observable<NewUserAddress> {
    return new Observable((observer) => {
      ...
      this.agmGeocoder.geocode({ location }).subscribe((result) => {
        if (result.length > 0) {
  ...
  registerNewUser(newUser: NewUser): Observable<boolean> {
    return new Observable((observer) => {
      ...
      this.httpClient.post<Response>(url, data).subscribe((response) => {
        if (response.ok) {
          observer.next(true);
        } else {
