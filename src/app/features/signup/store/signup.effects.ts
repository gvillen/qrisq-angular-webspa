import { LatLng } from './../schema/latLng';
...
import {
  ...
  actionFetchSubscriptionPlanListRequestSuccess,
...
import { SubscriptionPlan } from './signup.models';
import { Store } from '@ngrx/store';
import { selectSignUp } from './signup.selectors';
...
export class SignUpEffects {
  ...
  effectFetchSubscriptionPlanListRequest = createEffect(() =>
    this.actions$.pipe(
      ofType(actionFetchSubscriptionPlanListRequest),
      switchMap((action) =>
        this.signUpService.fetchSubscriptionPlans().pipe(
          map((response: Array<SubscriptionPlan>) =>
            actionFetchSubscriptionPlanListRequestSuccess({
  ...
  effectCreateAccountRequest = createEffect(() =>
    this.actions$.pipe(
      ...
      switchMap((action) =>
        this.signUpService.createAccount(action.signUp).pipe(
          take(1),
